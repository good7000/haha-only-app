...
with tab1:
    if not supabase: st.error("Connecting to server...")
    else:
        try:
            vids = supabase.table("videos").select("*").order("id", desc=True).execute().data
            if not vids: st.info("No videos yet.")
            for v in vids:
                st.subheader(v.get('title', 'Joy'))
                st.video(v['url'])
                col1, col2 = st.columns([4, 1])
                col1.caption(f"By: @{v.get('uploader_name', 'Guest')} | 🤣 {v.get('haha_count', 0)}")
                ...
                st.divider()
        except Exception as e: st.warning(f"Connecting... ({e})")
    ...
    if not st.session_state.logged_in: st.warning("Please Login to upload.")
        ...
        with st.form("upload_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Post 🚀"):
                supabase.table("videos").insert({"title": t, "url": url, "uploader_name": st.session_state.username, "haha_count": 0}).execute()
