...
st.markdown(f"""
    ...
    .dl-btn {{
        background-color: #FFD300; color: black; padding: 8px 16px;
    ...
    <div class="yellow-bar">
        <div class="marquee">Welcome to Ha Ha only! Share your funny videos and spread the joy around the world! 🤣 🚀 🤣 🚀</div>
    </div>
...
if 'joy_feed' not in st.session_state:
    st.session_state.joy_feed = [
        {"title": "Welcome Joy! 🤣", "url": "https://www.w3schools.com/html/mov_bbb.mp4", "likes": 10, "comment": "Admin: Let's start laughing!", "reports": 0}
    ]
...
st.title("🎬 Joy Feed")
t1, t2 = st.tabs(["🔥 Watch & Laugh", "📤 Share Joy"])
...
with t1:
    for i, v in enumerate(st.session_state.joy_feed[::-1]):
        if v['reports'] < 5:
            ...
            c1.caption(f"🤣 {v['likes']} Laughs | 🚩 {v['reports']} Reports")
            ...
            with c3:
                # Download Button
                st.markdown(f'<a href="{v["url"]}" download class="dl-btn">📥 Download</a>', unsafe_allow_html=True)
...
with t2:
    st.subheader("Post Joy for Everyone! 📤")
    with st.form("public_post", clear_on_submit=True):
        t = st.text_input("Video Title")
        u = st.text_input("Video Direct Link (.mp4)")
        c = st.text_area("Your Comment/Joke")
        if st.form_submit_button("Post Joy 🚀"):
            if t and u:
                st.session_state.joy_feed.append({"title": t, "url": u, "likes": 0, "comment": c, "reports": 0})
